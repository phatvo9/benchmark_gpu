...
class VLLMLlamaModel(OpenAIModelClass):
  ...
  def load_model(self):
    ...
    print(f'http://{server_args.get("host")}:{server_args.get("port")}/v1')
    self.client = OpenAI(
            api_key="notset",
            base_url=f'http://{server_args.get("host")}:{server_args.get("port")}/v1')
    self.model = model_id or self.client.models.list().data[0].id
    print(self.model)
